...
if ($dllSource -eq $null) {
    $devDll = Join-Path $repoRoot "src\Antigravity.Main\bin\Debug\Antigravity.Main.dll"
    if (Test-Path $devDll) {
        $dllSource = $repoRoot  # development source lives under repository src\
        $sourceMode = "DEVELOPMENT (Build từ Visual Studio - Debug)"
    }
    
    # Thử Release nếu Debug không có
    if ($dllSource -eq $null) {
        $relDll = Join-Path $repoRoot "src\Antigravity.Main\bin\Release\Antigravity.Main.dll"
        if (Test-Path $relDll) {
            $dllSource = $repoRoot
...
function Get-DllPath($modName) {
    ...
    $debugPath   = Join-Path $repoRoot "src\Antigravity.$modName\bin\Debug\Antigravity.$modName.dll"
    $releasePath = Join-Path $repoRoot "src\Antigravity.$modName\bin\Release\Antigravity.$modName.dll"
