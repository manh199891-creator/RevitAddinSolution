...
3. **Verification:**
   A temporary `test_serialize.cs` diagnostic probe was used to verify that the updated DLL handles local, unspecified, and UTC date edge cases without throwing exceptions. The one-off probe was retired during the 2026-08-23 root cleanup after its result had been preserved in this report.
...
2. **Deploy the Updated DLL:**
   * Run the deployment script [scripts/deploy/Deploy-ToRevit.ps1](file:///e:/Antigravity/RevitAddinSolution/scripts/deploy/Deploy-ToRevit.ps1) to copy the newly compiled DLLs to the Revit add-in folder.
3. **Verify Saving:**
